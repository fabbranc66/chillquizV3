...
setButton(btnRisultati, sessione.stato !== 'conclusa');    setButton(btnProssima, sessione.stato === 'risultati');
...
async function aggiornaPartecipanti() {
    ...
    if (data.success) {
        partecipantiSpan.textContent = data.classifica.length;
    }
}
...
function aggiornaTimer(sessione) {
    ...
    const durata = 120; // puoi leggerlo da config se vuoi
