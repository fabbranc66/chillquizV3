...
function tokenize(string $value): array
{
    ...
    $stopwords = ['quale','quali','chi','come','dove','quando','del','della','delle','degli','dei','nel','nella','nelle','negli','nello','che','e','il','la','lo','i','gli','le','di','a','da','per','con','in','su','tra','fra','si','piu','generale','cultura'];
    $out = [];
...
{
    $chunks = ['t:' . norm((string) $q['text']), 'a:19', 'y:media'];
    $ops = [];
...
$questions = json_decode((string) file_get_contents(BASE_PATH . '/storage/general_knowledge_questions.json'), true);
if (!is_array($questions)) {
    throw new RuntimeException('Dataset cultura generale non valido');
}

$existingStmt = $pdo->query("SELECT id, testo, fingerprint_unico FROM domande WHERE argomento_id = 19");
...
$insertDomandaStmt = $pdo->prepare("INSERT INTO domande (testo, codice_domanda, fingerprint_unico, difficolta, tipo_domanda, fase_domanda, media_image_path, argomento_id, attiva) VALUES (:testo, '', :fingerprint_unico, :difficolta, 'MEDIA', 'domanda', NULL, 19, 1)");
...
foreach ($questions as $question) {
    ...
    try {
        ...
        $updateCodeStmt->execute([
            'codice' => sprintf('MED-A019-%05d', $domandaId),
            'id' => $domandaId,
        ]);
        ...
        if ($imageSource !== '') {
            ...
            if ($binary !== null) {
                ...
                $filename = 'gnk_' . $domandaId . '_' . slugify((string) $question['image_term']) . '.' . $ext;
