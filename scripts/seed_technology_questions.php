...
function tokenize(string $value): array
{
    ...
    $stopwords = [
        ...
        'nel','nella','nelle','negli','nello','che','e','il','la','lo','i','gli','le','di','a','da',
        'per','con','in','su','tra','fra','si','detto','detta','chiamato','chiamata','tecnologia',
        'digitale','informatico','computer','sistema','rete','dispositivo','software','hardware',
        'programma','standard','protocollo','linguaggio','piattaforma','memoria','dati','file'
    ];
    $out = [];
...
$questionsPath = BASE_PATH . '/storage/technology_questions.json';
$questions = json_decode((string) file_get_contents($questionsPath), true);
if (!is_array($questions)) {
    throw new RuntimeException('Dataset tecnologia non valido');
}

$existingStmt = $pdo->query("SELECT id, testo, fingerprint_unico FROM domande WHERE argomento_id = 4");
...
$insertDomandaStmt = $pdo->prepare("INSERT INTO domande (testo, codice_domanda, fingerprint_unico, difficolta, tipo_domanda, fase_domanda, media_image_path, argomento_id, attiva) VALUES (:testo, :codice_domanda, :fingerprint_unico, :difficolta, 'MEDIA', 'domanda', NULL, 4, 1)");
...
foreach ($questions as $question) {
    ...
    try {
        ...
        $codice = sprintf('MED-A004-%05d', $domandaId);
        ...
        if ($imageSource !== '') {
            ...
            if ($binary !== null) {
                ...
                $filename = 'tec_' . $domandaId . '_' . slugify((string)$question['image_page']) . '.' . $ext;
