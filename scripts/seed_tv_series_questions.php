...
function tokenize(string $value): array
{
    ...
    $stopwords = ['quale','quali','chi','come','dove','quando','del','della','delle','degli','dei','nel','nella','nelle','negli','nello','che','e','il','la','lo','i','gli','le','di','a','da','per','con','in','su','tra','fra','si','serie','tv','televisiva'];
    $out = [];
...
{
    $chunks = ['t:' . norm((string) $q['text']), 'a:13', 'y:media'];
    $ops = [];
    foreach ($q['options'] as $opt) {
        $ops[] = (($opt['corretta'] ?? false) ? '1:' : '0:') . norm((string) ($opt['text'] ?? ''));
    }
...
$questions = json_decode((string) file_get_contents(BASE_PATH . '/storage/tv_series_questions.json'), true);
if (!is_array($questions)) throw new RuntimeException('Dataset TV e Serie non valido');

$existingStmt = $pdo->query("SELECT id, testo, fingerprint_unico FROM domande WHERE argomento_id = 13");
...
$insertDomandaStmt = $pdo->prepare("INSERT INTO domande (testo, codice_domanda, fingerprint_unico, difficolta, tipo_domanda, fase_domanda, media_image_path, argomento_id, attiva) VALUES (:testo, '', :fingerprint_unico, :difficolta, 'MEDIA', 'domanda', NULL, 13, 1)");
...
$inserted = 0; $skipped = 0; $imageOk = 0; $imageFail = 0; $report = [];
foreach ($questions as $question) {
    $fp = fingerprint($question);
    $questionTokens = tokenize((string) $question['text']);
    if (isset($existingFingerprints[$fp])) {
        $skipped++;
        $report[] = ['status' => 'skip-fingerprint', 'text' => $question['text'], 'id' => null];
        continue;
    }
    ...
    foreach ($existingTokenMaps as $existingId => $existingTokens) {
        ...
        if ($score >= 0.72 || (count($common) >= 5 && $score >= 0.55)) {
            ...
            $report[] = ['status' => 'skip-token', 'text' => $question['text'], 'id' => $existingId, 'score' => round($score, 3)];
            break;
        }
    }
    ...
    try {
        $insertDomandaStmt->execute(['testo' => $question['text'], 'fingerprint_unico' => $fp, 'difficolta' => '1.0']);
        $domandaId = (int) $pdo->lastInsertId();
        $updateCodeStmt->execute(['codice' => sprintf('MED-A013-%05d', $domandaId), 'id' => $domandaId]);
        foreach ($question['options'] as $option) {
            $insertOpzioneStmt->execute(['domanda_id' => $domandaId, 'testo' => (string) ($option['text'] ?? ''), 'corretta' => !empty($option['corretta']) ? 1 : 0]);
        }
        $summaryUrl = 'https://en.wikipedia.org/api/rest_v1/page/summary/' . rawurlencode((string) $question['image_term']);
        ...
        if ($imageSource !== '') {
            ...
            if ($binary !== null) {
                ...
                $filename = 'tvs_' . $domandaId . '_' . slugify((string) $question['image_term']) . '.' . $ext;
        ...
        $report[] = ['status' => 'insert', 'text' => $question['text'], 'id' => $domandaId];
    } catch (Throwable $e) {
        $pdo->rollBack();
        $report[] = ['status' => 'error', 'text' => $question['text'], 'message' => $e->getMessage()];
    }
}
